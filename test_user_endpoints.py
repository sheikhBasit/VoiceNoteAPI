    ...
    try:
        ...
        if make_admin:
            ...
            if admin_response.status_code == 200:
                ...
                log_test("auth", False, f"Admin promotion failed: {admin_response.text}")
...
def test_3_search_users():
    ...
    try:
        # Search by name
        response = requests.get(
            f"{BASE_URL}/users/search?query=Test",
            timeout=10
        )
        ...
        if response.status_code == 200:
            ...
            log_test("search_users", True, f"Found {count} users matching 'Test'", duration)
        else:
...
def test_6_admin_get_user_details():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_test("admin_get_user_details", True, f"Retrieved detailed user info", duration)
            ...
            log_test("admin_user_details_structure", structure_ok, "All detail sections present")
        else:
            log_test("admin_get_user_details", False, f"Request failed: {response.status_code}")
...
def test_7_admin_list_users():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_test("admin_list_users", True, f"Retrieved {users_count} users (total: {total})", duration)
        else:
            log_test("admin_list_users", False, f"Request failed: {response.status_code}")
...
def test_8_admin_user_statistics():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_test("admin_user_stats", False, f"Request failed: {response.status_code}")
...
def test_9_admin_get_user_devices():
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/admin/users/{user_id}/devices",
            headers=headers,
            timeout=10
        )
        ...
        if response.status_code == 200:
            ...
            log_test("admin_get_user_devices", True, f"Retrieved {devices_count} devices", duration)
        else:
            log_test("admin_get_user_devices", False, f"Request failed: {response.status_code}")
...
def test_10_admin_restore_user():
    ...
    try:
        ...
        if response.status_code == 200:
            result = response.json()
            log_test("admin_restore_user", True, f"User restored successfully", duration)
        else:
            log_test("admin_restore_user", False, f"Request failed: {response.status_code}")
...
def test_11_admin_hard_delete_user():
    ...
    try:
        ...
        if response.status_code == 200:
            result = response.json()
            log_test("admin_hard_delete_user", True, f"User permanently deleted", duration)
            
            deleted_items = result.get("deleted_items", {})
            log_test("admin_hard_delete_items", True, 
                    f"Deleted items: {deleted_items.get('notes', 0)} notes, {deleted_items.get('tasks', 0)} tasks")
        else:
            log_test("admin_hard_delete_user", False, f"Request failed: {response.status_code}")
...
def test_12_list_all_admins():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_test("list_admins", True, f"Retrieved {admin_count} admin users", duration)
        else:
