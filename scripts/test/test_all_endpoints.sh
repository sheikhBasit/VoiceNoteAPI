...
test_endpoint() {
    ...
    # Make the request
    local h_args=("-H" "X-Force-Signature-Check: true")
    if [ ! -z "$headers" ]; then
        h_args+=("-H" "$headers")
    fi
    
    if [ -z "$data" ]; then
        response=$(curl -s -w "\n%{http_code}" -X "$method" "$BASE_URL$endpoint" "${h_args[@]}")
    else
...
# Upgrade user to PREMIUM and ADMIN for tests
docker exec voicenote_db psql -U postgres -d voicenote -c "UPDATE users SET tier = 'PREMIUM', is_admin = true WHERE email = '$USER_EMAIL';" > /dev/null 2>&1 || true
...
UPDATE_USER_PAYLOAD='{"name":"Updated Name","timezone":"UTC"}'
test_endpoint "Update User" "PATCH" "/api/v1/users/me" "200" "$UPDATE_USER_PAYLOAD" "$AUTH_HEADER"

test_endpoint "Search Users" "GET" "/api/v1/users/search?query=test" "200" "" "$AUTH_HEADER"
...
test_endpoint "Search Notes" "POST" "/api/v1/notes/search" "200" '{"query":"test"}' "$AUTH_HEADER"
...
CREATE_TASK_PAYLOAD='{"description":"Test Task","priority":"MEDIUM","deadline":null,"assigned_entities":[],"image_uris":[],"document_uris":[],"external_links":[],"communication_type":"WHATSAPP","is_action_approved":false}'
...
test_endpoint "Search Tasks" "GET" "/api/v1/tasks/search?query_text=test" "200" "" "$AUTH_HEADER"
...
test_endpoint "AI Search" "POST" "/api/v1/ai/search?query=test" "200" "" "$AUTH_HEADER"
...
test_endpoint "Invalid Token" "GET" "/api/v1/notes" "401" "" "Authorization: Bearer invalid_token"
