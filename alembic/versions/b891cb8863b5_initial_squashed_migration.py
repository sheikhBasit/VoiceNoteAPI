...
def upgrade() -> None:
    ...
    # Note: org_id FK requires organizations table, but organizations requires users.
    # So we must create users first with NULL org_id, then create organizations, then add FK?
    # Or rely on DEFERRABLE INITIALLY DEFERRED?
    # Postgres doesn't support recursive FKs easily in one step if both are NOT NULL.
    # But here org_id is nullable.
    # Wait, 'organizations' is NOT created yet. So FK to it will fail.
    # Strategy: Create users WITHOUT ForeignKey to organizations first.
    # Then create organizations.
    # Then add FK to users.
    # But Alembic 'op.create_table' defines constraints inline.
    # We must REMOVE 'org_id' FK from here and add it via op.create_foreign_key LATER.
    ...
    # NOW start adding the cross-dependencies
    op.create_foreign_key('users_org_id_fkey', 'users', 'organizations', ['org_id'], ['id'], ondelete='SET NULL')
...
def downgrade() -> None:
    ...
    # Drop organizations BEFORE users because of FK dependency (admin_user_id)
    ...
    # Drop Enums to allow clean upgrade
    # Use raw SQL with CASCADE to force removal of types
    op.execute("DROP TYPE IF EXISTS subscriptiontier CASCADE")
    op.execute("DROP TYPE IF EXISTS userrole CASCADE")
    op.execute("DROP TYPE IF EXISTS notestatus CASCADE")
