...
class CurlTester:
    ...
    def test(self, name: str, method: str, endpoint: str, expected_codes: List[int],
             data: dict = None, headers: dict = None, params: str = "") -> Optional[str]:
    ...
    def create_test_note(self) -> bool:
        ...
        status, response = self.curl_request("POST", "/api/v1/notes/create", data,
    ...
    def create_test_task(self) -> bool:
        ...
        status, response = self.curl_request("POST", "/api/v1/tasks", data,
                                            {"Authorization": f"Bearer {self.token}"})
        try:
            resp_json = json.loads(response)
    ...
    def run_all_tests(self):
        ...
        if self.note_id:
            ...
            self.test("Update Note", "PATCH", f"/api/v1/notes/{self.note_id}", [200],
                      data={"title": "Updated Title", "content": "Updated content"})
            self.test("Get WhatsApp Draft", "GET", f"/api/v1/notes/{self.note_id}/whatsapp", [200])
            self.test("Semantic Analysis", "POST", f"/api/v1/notes/{self.note_id}/semantic-analysis", [200, 202])
        ...
        if self.task_id:
            ...
            self.test("Update Task", "PATCH", f"/api/v1/tasks/{self.task_id}", [200],
                      data={"description": "Updated Task", "priority": "HIGH"})
            self.test("Duplicate Task", "POST", f"/api/v1/tasks/{self.task_id}/duplicate", [200, 201])
            self.test("Delete Task", "DELETE", f"/api/v1/tasks/{self.task_id}", [200, 204])
        ...
        self.test("AI Search", "POST", "/api/v1/ai/search", [200, 429], params="?query=test")
    ...
    def print_summary(self):
        ...
        if self.failed == 0:
            ...
            print(f"{GREEN}╚════════════════════════════════════════════════════════════════════╝{RESET}\n")
            print(f"{GREEN}🎉 DEPLOYMENT READY - All endpoints verified and working!{RESET}\n")
        else:
            print(f"{RED}╔════════════════════════════════════════════════════════════════════╗{RESET}")
...
if __name__ == "__main__":
    ...
    print(f"{BLUE}║               All Tests Using curl Commands                       ║{RESET}")
    print(f"{BLUE}╚════════════════════════════════════════════════════════════════════╝{RESET}")
