...
class StorageService:
    ...
    def generate_presigned_put_url(self, object_name: str, expires_delta: int = 3600):
        ...
        try:
            url = self.client.presigned_put_object(
                self.bucket_name,
                object_name,
                expires=timedelta(seconds=expires_delta)
            )
            return url
        except Exception as e:
            JLogger.error("Failed to generate pre-signed URL", error=str(e), object_name=object_name)
            raise e
    ...
    def download_file(self, object_name: str, local_path: str):
        ...
        try:
            ...
            JLogger.info("File downloaded from storage", object_name=object_name, local_path=local_path)
            return local_path
        except Exception as e:
            JLogger.error("Failed to download file from storage", error=str(e), object_name=object_name)
            raise e
    ...
    def delete_file(self, object_name: str):
        ...
        try:
            ...
            JLogger.error("Failed to delete file from storage", error=str(e), object_name=object_name)
