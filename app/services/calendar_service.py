...
class CalendarService:
    ...
    def get_user_events(user_id: str) -> List[Dict[str, Any]]:
        ...
        return [
            {"title": "Gym Session", "start_time": int(time.time() * 1000) + 3600000, "end_time": int(time.time() * 1000) + 7200000},
            {"title": "Lunch", "start_time": int(time.time() * 1000) + 10800000, "end_time": int(time.time() * 1000) + 14400000}
        ]
