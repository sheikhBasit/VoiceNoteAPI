...
class NotificationService:
    ...
    def initialize(cls):
        ...
        try:
            ...
            if cred:
                ...
                JLogger.warning("Firebase credentials not found. Push notifications will be mocked.")
    ...
    def send_multicast(tokens: list, title: str, body: str, data: dict = None):
        ...
        if not NotificationService._initialized:
            JLogger.info("Mocking Push Notification (Service not init)", count=len(tokens), title=title)
        ...
        try:
            ...
            JLogger.info("Push notification multicast sent", 
    ...
    def send_to_token(token: str, title: str, body: str, data: dict = None):
        ...
        if not NotificationService._initialized:
            JLogger.info("Mocking Push Notification (Service not init)", token=token[-10:], title=title)
        ...
        try:
            ...
            JLogger.error("Failed to send push notification", token=token[-10:], error=str(e))
            raise e
