...
class MeetingService:
    ...
    def create_bot(
        self, meeting_url: str, bot_name: str, user_id: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            resp = requests.post(
                f"{self.BASE_URL}/bot", json=payload, headers=self.headers
            )
    ...
    def handle_webhook_event(self, event_data: Dict[str, Any]):
        ...
        if event_type == "bot.status_change":
            ...
            if user_id:
                broadcast_ws_update(
                    user_id, "BOT_STATUS", {"bot_id": bot_id, "status": status}
                )
            ...
            if user_id and transcript:
                broadcast_ws_update(
                    user_id, "LIVE_TRANSCRIPT", {"bot_id": bot_id, "text": transcript}
                )
    ...
    def _save_transcript_and_summarize(
        self, user_id: str, transcript: str, bot_id: str
    ):
        try:
            ...
            logger.info(
                f"Meeting ended. Automated synthesis triggered for user {user_id} (Note ID: {new_note.id})"
            )
        except Exception as e:
