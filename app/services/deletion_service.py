...
from app.db.models import User, Note, Task
...
class DeletionService:
    ...
    def soft_delete_user(
        ...
        """
        JLogger.info("Starting soft delete user", user_id=user_id, deleted_by=deleted_by)
        ...
        try:
            ...
            db.rollback()
            JLogger.error("Error during user soft delete", user_id=user_id, error=str(e))
    ...
    def soft_delete_note(
        ...
        Returns:
            dict: Deletion summary with counts
        """
        JLogger.info("Starting soft delete note", note_id=note_id, deleted_by=deleted_by)
        ...
        try:
            ...
            tasks_count = db.query(Task).filter(
                and_(Task.note_id == note_id, Task.is_deleted == False)
            ).count()
            ...
            db.rollback()
            JLogger.error("Error during note soft delete", note_id=note_id, error=str(e))
            return {"success": False, "error": str(e)}
    ...
    def soft_delete_task(
        ...
        """
        JLogger.info("Starting soft delete task", task_id=task_id, deleted_by=deleted_by)
    ...
    def restore_user(db: Session, user_id: str, restored_by: str) -> Dict[str, Any]:
        ...
        try:
            ...
            notes = db.query(Note).filter(
                and_(Note.user_id == user_id, Note.is_deleted == True, Note.can_restore == True)
            ).all()
            ...
            for note in notes:
                ...
                tasks = db.query(Task).filter(
                    and_(Task.note_id == note.id, Task.is_deleted == True, Task.can_restore == True)
                ).all()
            ...
            db.rollback()
            JLogger.error("Error during user restoration", user_id=user_id, error=str(e))
            return {"success": False, "error": str(e)}
    ...
    def restore_note(db: Session, note_id: str, restored_by: str) -> Dict[str, Any]:
        ...
        try:
            note = db.query(Note).filter(Note.id == note_id).first()
            if not note: return {"success": False, "error": "Note not found"}
            if not note.is_deleted: return {"success": False, "error": "Note not deleted"}
            ...
            # Restore tasks
            tasks = db.query(Task).filter(
                and_(Task.note_id == note_id, Task.is_deleted == True, Task.can_restore == True)
            ).all()
            ...
            db.rollback()
            JLogger.error("Error during note restoration", note_id=note_id, error=str(e))
            return {"success": False, "error": str(e)}
    ...
    def restore_task(db: Session, task_id: str, restored_by: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not task.is_deleted: return {"success": False, "error": "Task not deleted"}
            ...
            db.rollback()
            JLogger.error("Error during task restoration", task_id=task_id, error=str(e))
            return {"success": False, "error": str(e)}
