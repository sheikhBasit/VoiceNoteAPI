...
class AudioChunker:
    ...
    def should_chunk(file_path: str, max_duration_minutes: int = 5) -> bool:
        ...
        try:
            JLogger.debug(
                "Checking if audio should be chunked",
                file_path=file_path,
            ...
            JLogger.error(
                "Error checking audio duration", file_path=file_path, error=str(e)
            )
            return False
    ...
    def validate_audio_file(file_path: str) -> tuple[bool, str]:
        ...
        if file_size > 100 * 1024 * 1024:
            JLogger.warning(
                "File too large", file_path=file_path, size_mb=file_size / 1024 / 1024
            )
    ...
    def chunk_audio(file_path: str, output_dir: str) -> list[str]:
        ...
        # Load audio
        JLogger.info(
            "Starting audio chunking", file_path=file_path, output_dir=output_dir
        )
        audio = AudioSegment.from_file(file_path)
        total_duration = len(audio)

        JLogger.info(
            "Audio loaded for chunking",
            file_path=file_path,
        ...
        JLogger.info(
            f"Target: {num_chunks} chunks of ~{target_chunk_duration/1000:.2f}s each"
        )
