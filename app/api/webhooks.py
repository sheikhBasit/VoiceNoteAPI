...
def _handle_invoice_payment(invoice, billing_service: BillingService):
    ...
    wallet = (
        billing_service.db.query(Wallet)
        .filter(Wallet.stripe_customer_id == customer_id)
        .first()
    )

    if wallet:
        logger.info(
            f"Subscription renewal for customer {customer_id} (User: {wallet.user_id}): +{amount_paid}"
        )
        billing_service.process_deposit(
            wallet.user_id, amount_paid, "Stripe Subscription"
        )
    else:
