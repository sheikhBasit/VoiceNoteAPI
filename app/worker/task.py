...
def note_process_pipeline(
    ...
    try:
        ...
        conflicts = []  # Initialize for linting safety
        ...
        try:
            ...
            if user:
                ...
                user.usage_stats["total_notes"] = (
                    user.usage_stats.get("total_notes", 0) + 1
                )
                user.usage_stats["total_tasks"] = user.usage_stats.get(
                    "total_tasks", 0
                ) + len(analysis.tasks)
            ...
            if not success:
                JLogger.warning(
                    "Insufficient funds or ledger failure for note processing",
                    user_id=note.user_id,
                    note_id=note_id,
                )
            else:
                JLogger.info(
                    "Usage charged successfully",
                    user_id=note.user_id,
        ...
        for t_data in analysis.tasks:
            ...
            actions_metadata = (
                t_data.get("actions", {}) if isinstance(t_data, dict) else {}
            )
            suggested_actions = {}

            # Generate Google Search action
            if "google_search" in actions_metadata:
                suggested_actions["google_search"] = (
                    ActionGenerator.generate_google_search(
            ...
            if "whatsapp" in actions_metadata:
                ...
                suggested_actions["whatsapp"] = (
                    ActionGenerator.generate_whatsapp_message(
            ...
            new_task = Task(
                ...
                deadline=(
                    t_data["deadline"] if isinstance(t_data, dict) else t_data.deadline
                ),
        ...
        if user and extracted_tasks:
            ...
            for conflict in all_conflicts:
                ...
                if user.authorized_devices:
                    device_token = user.authorized_devices[0].get(
                        "biometric_token", "mock_token"
                    )

                msg_prefix = (
                    "📅 Schedule" if conflict["type"] == "SCHEDULE" else "⚠️ Factual"
                )
        ...
        JLogger.info(
            "Worker: Note processing pipeline finished successfully", note_id=note_id
        )
        ...
        JLogger.warning(
            "Worker: AI Service Error (Likely silent or invalid audio)",
            note_id=note_id,
            error=str(e),
        )
        db.query(Note).filter(Note.id == note_id).update(
        ...
        broadcast_ws_update(
            note_owner_id,
            "NOTE_STATUS",
            {"note_id": note_id, "status": "DONE", "message": str(e)},
        )
...
def hard_delete_expired_records():
    ...
    try:
        ...
        expired_tasks = (
            db.query(Task)
            .filter(Task.is_deleted == True, Task.updated_at < cutoff_time)
            .all()
        )
        ...
        expired_notes = (
            db.query(Note)
            .filter(Note.is_deleted == True, Note.updated_at < cutoff_time)
            .all()
        )
        ...
        expired_users = (
            db.query(User)
            .filter(User.is_deleted == True, User.deleted_at < cutoff_time)
            .all()
        )
