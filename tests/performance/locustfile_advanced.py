...
class NormalUserTasks(TaskSet):
    ...
    def create_note(self):
        ...
        with self.client.post(
            "/api/notes", json=payload, catch_response=True
        ) as response:
    ...
    def ask_ai(self):
        ...
        with self.client.post(
            "/api/ai/ask", json=payload, catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
...
class PowerUserTasks(TaskSet):
    ...
    def get_notes_paginated(self):
        ...
        with self.client.get(
            f"/api/notes?page={page}&limit=50", catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
    ...
    def search_notes(self):
        ...
        with self.client.get(
            f"/api/notes/search?q={query}", catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
...
def on_test_stop(environment, **kwargs):
    ...
    print(
        f"Error Rate: {(report['errors']/report['count']*100) if report['count'] > 0 else 0:.2f}%"
    )
