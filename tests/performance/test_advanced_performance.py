...
class TestLoadAndLatency:
    ...
    def test_throughput_under_sustained_load(self):
        ...
        while time.time() - start_time < duration:
            ...
            while (
                time.time() - second_start < 1.0 and time.time() - start_time < duration
            ):
        ...
        stdev_throughput = (
            statistics.stdev(throughput_per_second)
            if len(throughput_per_second) > 1
            else 0
        )
...
class TestErrorHandling:
    ...
    def test_malformed_json_handling(self):
        ...
        from app.utils.ai_service_utils import AIServiceError, validate_json_response
    ...
    def test_error_response_consistency(self):
        ...
        if cv >= 1.5:
            pytest.skip(
                f"Skipping due to high timing variance (CV={cv:.2f}) - likely system jitter"
            )
