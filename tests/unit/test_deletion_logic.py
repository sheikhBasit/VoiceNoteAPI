...
def setup_db():
    ...
    yield
    # models.Base.metadata.drop_all(bind=engine) # Commended out to avoid circular dependency issues during drop
...
def test_user(db):
    ...
    user = models.User(
        id=user_id, name="Test User", email=f"test_{uuid.uuid4()}@example.com"
    )
    db.add(user)
    db.commit()
    db.refresh(user)
...
def test_note(db, test_user):
    ...
    note = models.Note(
        id=note_id, user_id=test_user.id, title="Test Note", summary="Test Summary"
    )
    db.add(note)
    db.commit()
    db.refresh(note)
...
def test_task(db, test_note):
    ...
    task = models.Task(id=task_id, note_id=test_note.id, description="Test Task")
    db.add(task)
    db.commit()
    db.refresh(task)
    return task
