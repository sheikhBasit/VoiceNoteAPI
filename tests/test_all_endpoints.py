...
class TestNoteEndpoints:
    ...
    def test_note_create(self, auth_header):
        ...
        response = client.post("/api/v1/notes/create", json=payload, headers=auth_header)
    ...
    def test_note_update(self, auth_header, test_note):
        ...
        response = client.patch(f"/api/v1/notes/{test_note.id}", json=payload, headers=auth_header)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_note_semantic_analysis(self, auth_header, test_note):
        ...
        response = client.post(f"/api/v1/notes/{test_note.id}/semantic-analysis", headers=auth_header)
        assert response.status_code in [200, 202]
    
    def test_note_whatsapp_draft(self, auth_header, test_note):
        """GET /api/v1/notes/{note_id}/whatsapp - Get WhatsApp draft"""
        response = client.get(f"/api/v1/notes/{test_note.id}/whatsapp", headers=auth_header)
        assert response.status_code == 200
        data = response.json()
...
class TestTaskEndpoints:
    ...
    def test_task_search(self, auth_header):
        ...
        response = client.get("/api/v1/tasks/search?query_text=test", headers=auth_header)
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)
    ...
    def test_task_update(self, auth_header, test_task):
        ...
        response = client.patch(f"/api/v1/tasks/{test_task.id}", json=payload, headers=auth_header)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_task_duplicate(self, auth_header, test_task):
        ...
        response = client.post(f"/api/v1/tasks/{test_task.id}/duplicate", headers=auth_header)
        assert response.status_code == 201
        data = response.json()
        assert "id" in data
...
class TestAIEndpoints:
    ...
    def test_ai_search(self, auth_header):
        ...
        response = client.post("/api/v1/ai/search?query=test+search", headers=auth_header)
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)
