...
def authenticated_user():
    ...
    db_url = (
        "postgresql://postgres:password@db:5432/voicenote"
        if (os.path.exists("/.dockerenv") or os.environ.get("RUNNING_IN_DOCKER"))
        else "postgresql://postgres:password@localhost:5433/voicenote"
    )
...
class TestMinIOArchitecture:
    ...
    def test_processing_completion(self, authenticated_user, test_audio_file):
        ...
        for i in range(max_retries):
            response = requests.get(
                f"{BASE_URL}/notes/{note_id}", headers=authenticated_user["headers"]
            )
            assert response.status_code == 200
        ...
        pytest.skip(
            f"Processing timeout for note {note_id} - Celery worker may not be actively processing"
        )
