...
def auth_headers():
    ...
    resp = client.post("/api/v1/users/sync", json={
        "name": "Regular User",
    ...
    return {"Authorization": f"Bearer {resp.json()['access_token']}", "user_id": resp.json()['user']['id']}
...
class TestAdminUpdates:
    def test_admin_update_user(self, db, admin_auth):
        ...
        timestamp = int(time.time())
        target_resp = client.post("/api/v1/users/sync", json={
            "name": "Target User",
...
class TestNotesRestoration:
    def test_restore_note(self, db, auth_headers):
        # 1. Create Note
        note = client.post("/api/v1/notes/create", headers=auth_headers, json={"title": "To Delete"}).json()
        note_id = note["id"]
        ...
        # 3. Restore Note
        restore_resp = client.patch(f"/api/v1/notes/{note_id}/restore", headers=auth_headers)
        assert restore_resp.status_code == 200
        assert restore_resp.json()["is_deleted"] is False

    def test_bulk_delete_notes(self, auth_headers):
        n1 = client.post("/api/v1/notes/create", headers=auth_headers, json={"title": "Bulk 1"}).json()["id"]
        n2 = client.post("/api/v1/notes/create", headers=auth_headers, json={"title": "Bulk 2"}).json()["id"]
        
        # BULK DELETE with Body
        payload = {"note_ids": [n1, n2]}
        bulk_resp = client.request("DELETE", "/api/v1/notes", headers=auth_headers, json=payload)
...
class TestTasksRestoration:
    def test_task_lifecycle_and_bulk(self, auth_headers):
        t1 = client.post("/api/v1/tasks", headers=auth_headers, json={"description": "Task 1", "priority": "LOW"}).json()["id"]
        t2 = client.post("/api/v1/tasks", headers=auth_headers, json={"description": "Task 2", "priority": "LOW"}).json()["id"]
        
        # Toggle Complete
        comp_resp = client.patch(f"/api/v1/tasks/{t1}/complete", headers=auth_headers, json={"is_done": True})
        assert comp_resp.status_code == 200
        assert comp_resp.json()["is_done"] is True
        
        # Bulk Delete
        bulk_resp = client.request("DELETE", "/api/v1/tasks", headers=auth_headers, json={"task_ids": [t1, t2]})
