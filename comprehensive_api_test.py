...
class TestResult:
    ...
    def add(self, name: str, method: str, endpoint: str, expected_code: int, 
            actual_code: int, passed: bool, response: str = ""):
        self.total += 1
        if passed:
            self.passed += 1
...
def test_endpoint(result: TestResult, name: str, method: str, endpoint: str, 
                 ...
                 headers: dict = None, auth_token: str = None) -> Tuple[int, dict]:
    ...
    try:
        ...
        result.add(name, method, endpoint, expected_codes[0], status_code, 
                  passed, json.dumps(response_data))
...
def main():
    ...
    if not auth_token:
        print(f"{Colors.RED}❌ Could not get auth token. Skipping remaining tests.{Colors.END}")
    ...
    test_endpoint(result, "Update User", "PATCH", "/api/v1/users/me", 
                 [200], update_user, auth_token=auth_token)
    ...
    status, response = test_endpoint(result, "Create Note", "POST", "/api/v1/notes/create", 
                                    [201, 200], create_note, auth_token=auth_token)
    ...
    if note_id:
        ...
        test_endpoint(result, "Semantic Analysis", "POST", f"/api/v1/notes/{note_id}/semantic-analysis", 
    ...
    status, response = test_endpoint(result, "Create Task", "POST", "/api/v1/tasks", 
                                    [201, 200], create_task, auth_token=auth_token)
    ...
    test_endpoint(result, "Get Tasks Assigned to Me", "GET", "/api/v1/tasks/assigned-to-me", 
                 [200], auth_token=auth_token)
    
    test_endpoint(result, "Search Tasks", "GET", "/api/v1/tasks/search?q=test", 
                 [200], auth_token=auth_token)
    ...
    test_endpoint(result, "AI Search", "POST", "/api/v1/ai/search", 
                 [200], ai_search, auth_token=auth_token)
    ...
    test_endpoint(result, "Invalid Task Data", "POST", "/api/v1/tasks", 
                 [400], invalid_task, auth_token=auth_token)
    ...
    with open(REPORT_FILE, 'w') as f:
        ...
        f.write(f"Pass Rate: {result.passed * 100 // result.total if result.total > 0 else 0}%\n")
