...
def get_signature_headers(method, path, query, body, body_is_bytes=False):
    ...
    signature = hmac.new(DEVICE_SECRET.encode(), message.encode(), hashlib.sha256).hexdigest()
...
task_data = {
    "description": "Buy milk manually (Python)",
    "priority": "MEDIUM"
}
...
with open("test_python.wav", "rb") as f:
    ...
    resp = requests.post(f"{BASE_URL}/notes/process", files=files, data=data, headers=admin_headers)
