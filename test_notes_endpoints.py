...
class NotesTestSuite:
    ...
    def test_get_presigned_url(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.log_test("GET /presigned-url", "PASS",
                             f"Generated presigned URL (note_id: {data.get('note_id')})",
                             time.time() - start_time)
                return data.get("note_id"), data.get("upload_url")
            else:
                self.log_test("GET /presigned-url", "FAIL",
                             f"Status {response.status_code}: {response.text}",
                             time.time() - start_time)
                return None, None
                
        except Exception as e:
            self.log_test("GET /presigned-url", "FAIL", str(e),
                         time.time() - start_time)
            return None, None
    ...
    def test_process_note_with_file(self, audio_path):
        ...
        try:
            ...
            if response.status_code in [200, 202]:
                ...
                self.log_test("POST /process", "PASS",
                             f"Note processing started",
                             time.time() - start_time)
                return result.get("note_id"), result.get("task_id")
            else:
                self.log_test("POST /process", "FAIL",
                             f"Status {response.status_code}: {response.text[:200]}",
                             time.time() - start_time)
                return None, None
                
        except Exception as e:
            self.log_test("POST /process", "FAIL", str(e),
                         time.time() - start_time)
            return None, None
    ...
    def test_list_notes(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_test("GET /notes", "PASS",
                             f"Retrieved {count} notes",
                             time.time() - start_time)
                return notes
            else:
                self.log_test("GET /notes", "FAIL",
                             f"Status {response.status_code}: {response.text}",
                             time.time() - start_time)
                return None
                
        except Exception as e:
            self.log_test("GET /notes", "FAIL", str(e),
                         time.time() - start_time)
            return None
    ...
    def test_get_note(self, note_id):
        ...
        try:
            ...
            if response.status_code == 200:
                note = response.json()
                self.log_test("GET /notes/{note_id}", "PASS",
                             f"Retrieved note (title: {note.get('title', 'N/A')})",
                             time.time() - start_time)
                return note
            else:
                self.log_test("GET /notes/{note_id}", "FAIL",
                             f"Status {response.status_code}: {response.text}",
                             time.time() - start_time)
                return None
                
        except Exception as e:
            self.log_test("GET /notes/{note_id}", "FAIL", str(e),
                         time.time() - start_time)
            return None
    ...
    def test_update_note(self, note_id):
        ...
        try:
            ...
            if response.status_code == 200:
                note = response.json()
                self.log_test("PATCH /notes/{note_id}", "PASS",
                             f"Updated note (new title: {note.get('title')})",
                             time.time() - start_time)
                return note
            else:
                self.log_test("PATCH /notes/{note_id}", "FAIL",
                             f"Status {response.status_code}: {response.text}",
                             time.time() - start_time)
                return None
                
        except Exception as e:
            self.log_test("PATCH /notes/{note_id}", "FAIL", str(e),
                         time.time() - start_time)
            return None
    ...
    def test_search_notes(self):
        ...
        try:
            ...
            self.log_test("POST /notes/search", "FAIL", str(e),
                         time.time() - start_time)
            return None
    ...
    def test_get_dashboard_metrics(self):
        ...
        try:
            ...
            self.log_test("GET /notes/dashboard", "FAIL", str(e),
                         time.time() - start_time)
            return None
    ...
    def test_ask_ai(self, note_id):
        ...
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                self.log_test("POST /notes/{note_id}/ask", "PASS",
                             f"AI response received",
                             time.time() - start_time)
                return result
            else:
                self.log_test("POST /notes/{note_id}/ask", "FAIL",
                             f"Status {response.status_code}: {response.text}",
                             time.time() - start_time)
                return None
                
        except Exception as e:
            self.log_test("POST /notes/{note_id}/ask", "FAIL", str(e),
                         time.time() - start_time)
            return None
    ...
    def test_delete_note(self, note_id):
        ...
        try:
            ...
            if response.status_code in [200, 204]:
                self.log_test("DELETE /notes/{note_id}", "PASS",
                             f"Note deleted successfully",
                             time.time() - start_time)
                return True
            else:
                self.log_test("DELETE /notes/{note_id}", "FAIL",
                             f"Status {response.status_code}: {response.text}",
                             time.time() - start_time)
                return False
                
        except Exception as e:
            self.log_test("DELETE /notes/{note_id}", "FAIL", str(e),
                         time.time() - start_time)
            return False
